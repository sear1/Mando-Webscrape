...
(async () => {
    ...
    for(let mando of URLS) {
        ...
        //created promise for the finish result to print out the id of succeful download.
        .on('finish', () => {
            console.log(`${mando.id} has finished downloading the image.`);
            resolve();

        })
        //created error promise to print out errors on reject promise and with the id
        .on('error', (error) =>{
            reject(error);
        })
    })
    .catch(error =>{
        console.log(`${mando.id} has had an error on download.${error}`);
    });
    }
